pub mod solution {
    ...
    pub fn solve<R: std::io::Read, W: std::io::Write>(
        ...
        let mut a = vec![0; n + 1];
        let mut prefixes = vec![0; n + 1];
        let mut suffixes = vec![0; n + 2];
        ...
        let mut ans = 0;
        for i in 1usize..=n {
            ans = max(ans, gcd(prefixes[i - 1], suffixes[i + 1]));
        }
...
mod math {
    ...
    mod identities {
        /*
         * Copied from https://github.com/rust-num/num-traits/blob/master/src/identities.rs
         */
    ...
    pub fn gcd<
        N: PartialEq + PartialOrd + Add<N> + Sub<N> + Mul<N> + Rem<Output = N> + Zero + Copy,
    >(
        mut m: N,
        mut n: N,
    ) -> N {
