pub mod solution {
    ...
    pub fn solve<R: std::io::Read, W: std::io::Write>(
        ...
        let mut ans = sum[1];
        // At the beginning of each iteration of the following loop, this represents the minimum
        // value of `sum[l]` for all `l < i`.
        let mut running_minimum = 0;
        for i in 1usize..=n {
            ans = max(ans, sum[i] - running_minimum);
            running_minimum = min(running_minimum, sum[i]);
        }
