pub mod solution {
    ...
    pub fn solve<R: std::io::Read, W: std::io::Write>(
        ...
        for _ in 0usize..t {
            let w_r: i32 = reader.next();
            let h_r: i32 = reader.next();

            let t1_x_bl: i32 = reader.next();
            let t1_y_bl: i32 = reader.next();
            let t1_x_tr: i32 = reader.next();
            let t1_y_tr: i32 = reader.next();
            let t_1_w = t1_x_tr - t1_x_bl;
            let t_1_h = t1_y_tr - t1_y_bl;

            let t2_w: i32 = reader.next();
            let t2_h: i32 = reader.next();

            if (t2_w > (w_r - t_1_w)) && (t2_h > (h_r - t_1_h)) {
                writer.println("-1");
                continue;
            }

            let largest_x_gap = max(w_r - t1_x_tr, t1_x_bl);
            let largest_y_gap = max(h_r - t1_y_tr, t1_y_bl);
            let delta_x = max(0, t2_w - largest_x_gap);
            let delta_y = max(0, t2_h - largest_y_gap);

            if t2_w > (w_r - t_1_w) {
                writer.println(delta_y);
            } else if t2_h > (h_r - t_1_h) {
                writer.println(delta_x);
            } else {
                writer.println(min(delta_x, delta_y));
            }
        }
    }
}
