pub mod solution {
    ...
    pub fn solve<R: std::io::Read, W: std::io::Write>(
        ...
        const MAX_R: usize = 1000;

        let mut count = [0; MAX_R + 1];
        ...
        let mut sorted_data = (0usize..=MAX_R)
            .filter(|r| count[*r] > 0)
            .map(|r| FrequencyDatum {
                reading: r,
                frequency: count[r],
            })
            .collect::<Vec<FrequencyDatum>>();
        sorted_data.sort_by(|a, b| {
            if a.frequency == b.frequency {
                return b.reading.cmp(&a.reading);
            } else {
                return b.frequency.cmp(&a.frequency);
            }
        });

        let second_largest_frequency = sorted_data[1].frequency;
        let mut ans = 0;
        for datum in sorted_data.iter() {
            if datum.frequency == second_largest_frequency {
                ans = std::cmp::max(
                    ans,
                    ((sorted_data[0].reading as i32) - (datum.reading as i32)).abs(),
                );
            }
        }
        writer.println(ans);
