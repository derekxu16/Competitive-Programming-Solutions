pub mod solution {
    ...
    struct PowerGeneratingHouse {
        x: u64,
        index_in_all_houses: usize,
    }
    ...
    pub fn solve<R: std::io::Read, W: std::io::Write>(
        ...
        for _ in 0usize..t {
            ...
            // Indices of houses that generate power.
            let mut generate_power = Vec::new();
            // Indices of all houses.
            let mut all_houses = Vec::new();

            for i in 0usize..n {
                let x: u64 = reader.next();
                let p = reader.next::<u8>() != 0;
                if p {
                    generate_power.push(PowerGeneratingHouse {
                        x,
                        index_in_all_houses: i,
                    });
                }
                all_houses.push(x);
            }
            ...
            let mut ans = 0u64;
            if all_houses[0] < generate_power[0].x {
                ans += generate_power[0].x - all_houses[0];
            }
            if all_houses.last().unwrap() > &generate_power.last().unwrap().x {
                ans += all_houses.last().unwrap() - &generate_power.last().unwrap().x;
            }
            for i in 1usize..generate_power.len() {
                let left = &generate_power[i - 1];
                let right = &generate_power[i];
                let mut largest_gap_in_interval = 0;
                for j in (left.index_in_all_houses + 1)..=right.index_in_all_houses {
                    largest_gap_in_interval =
                        max(largest_gap_in_interval, all_houses[j] - all_houses[j - 1]);
                }
                ans += right.x - left.x - largest_gap_in_interval;
            }
