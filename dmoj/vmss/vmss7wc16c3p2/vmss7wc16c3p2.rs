pub mod solution {
    ...
    fn dfs(u: usize, goal: usize, graph: &Vec<Vec<usize>>, visited: &mut Vec<bool>) -> bool {
        if visited[u] {
            return false;
        }
        if u == goal {
    ...
    pub fn solve<R: std::io::Read, W: std::io::Write>(
        ...
        let m: u16 = reader.next();
        let a: u16 = reader.next();
        let b: u16 = reader.next();

        for _ in 0u16..m {
            let u: usize = reader.next();
            let v: usize = reader.next();
            graph[u].push(v);
            graph[v].push(u);
        }

        if dfs(a.into(), b.into(), &graph, &mut visited) {
            writer.println("GO SHAHIR!");
        } else {
            writer.println("NO SHAHIR!");
        }
    }
}
